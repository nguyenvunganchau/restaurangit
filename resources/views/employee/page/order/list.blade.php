            ...
            <div class="page-header">
                <div class="row align-items-end">
                    ...
                    <div class="col-sm-auto">
                        <a class="btn btn-primary" href="{{ route('show_create_order.index') }}">
                        ...
                        <tbody>
                            @foreach ($order as $key => $val)
                                <tr>
                                    ...
                                    <td>{{ $val ? 'DH' . $val->order_id : '' }}</td>
                                    <td>{{ $val->customer->name ?? '' }}</td>
                                    <td>{{ $val->customer->phone ?? '' }}</td>
                                    <td>{{ $val->table_type_order->name ?? '' }}</td>
                                    <td>{{ $val->table_order->name ?? '' }}</td>
                                    <td>
                                        {{ $val && $val->created_at ? $val->created_at->setTimezone('Asia/Ho_Chi_Minh')->format('H:i d/m/Y') : '' }}
                                    </td>
                                    <td>{{ $val ? $val->time : '' }} giờ</td>
                                    <td>{{ number_format($val ? $val->total_amount : '') }} VNĐ<span
                                            class="text-hide">Code:
                                            GB</span></td>
                                    <td>
                                        <a href="{{ route('pdf.get', $val->order_id) }}" class="btn btn-sm btn-success">
                                            <i class="tio-download"></i></a>
                                        <a class="btn btn-sm btn-primary"
                                            href="{{ route('show_update_order.index', $val->order_id) }}">
                                            <i class="tio-edit"></i>
                                        </a>
                                        {{-- <a class="btn btn-sm btn-danger" href="{{ route('order.delete', $val->order_id) }}"
