                        ...
                        <tbody>
                            @foreach ($menu as $key => $val)
                                <tr>
                                    ...
                                    <td>
                                        <div>
                                            <img src="{{ asset($val->image_menu) ? '' . Storage::url($val->image_menu) : $val->item_name }}"
                                                alt="" class="avatar-sm rounded-circle me-2">
                                        </div>
                                    </td>
                                    <td>{{ $val ? $val->item_name : '' }}</td>
                                    <td>{{ number_format($val->price) }}</td>
                                    <td>{{ $val ? number_format($val->discount) : '' }}</td>
                                    <td>{{ $val->category->name ?? '' }}</td>
                                    <td>{{ $val ? $val->description : '' }}</td>
                                    <td>
                                        <a class="btn btn-sm btn-primary"
                                            href="{{ route('edit_menu.get', $val->item_id) }}">
                                            <i class="tio-edit"></i>
                                        </a>
                                        {{-- <a class="btn btn-sm btn-danger" href="{{ route('menu.delete', $val->item_id) }}"
